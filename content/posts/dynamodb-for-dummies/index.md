...
* Tables - A table holds items, much like relational databases. However,
    ...
    of any "database". Tables do *not* have a rigid column structure.
...
items can have the same partition key. In order to query an item, one *must* know
the partition key.
...
combination of the partition key and sort key. However, the partition key does *not*
need to be unique. The sort key, must still be unique.
In order to query a single item, one *must* know
...
scanning. This really should be avoided if at all possible as it will consume a *LOT*
