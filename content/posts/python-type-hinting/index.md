...
may actually be defined _after_ the type hint itself, which causes an issue.
...
to your file(s). This effectively tells Python to evaluate type hints _later_, so the
...
VS Code with Pylance, _or_ you could setup an automated job to check every pull request
...
[^1]:
    Yes, in some extremely rare cases, this is not the case. One would have to override
...
[^3]:
    [Ironic](https://tenor.com/view/ironic-starwars-chode-gif-5274592) that Google
